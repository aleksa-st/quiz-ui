            ...
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="max-w-md mx-auto bg-white p-1 rounded-xl shadow-sm border border-slate-200 flex">
                    <button
                        ...
                        className={`flex-1 px-4 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'unlocked' ? 'bg-violet-50 text-violet-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Unlocked ({achievements.length})
                    </button>
                    <button
                        onClick={() => setActiveTab('all')}
                        className={`flex-1 px-4 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'all' ? 'bg-violet-50 text-violet-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {displayedAchievements.map(achievement => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`h-2 ${unlocked ? 'bg-gradient-to-r from-violet-500 to-fuchsia-500' : 'bg-slate-200'}`}></div>
                                    <CardContent className="p-6 flex flex-col items-center text-center h-full relative overflow-hidden">
                                        ...
                                        <div className={`p-5 rounded-2xl mb-4 transition-transform group-hover:scale-110 duration-300 ${unlocked ? 'bg-violet-50 shadow-inner' : 'bg-slate-200 grayscale'}`}>
                                            {getIcon(achievement.icon, unlocked ? 'text-violet-600' : 'text-slate-400')}
                                        </div>
